...
# ---------------------------------------------------------------------------
# SQL Dialect Transformer  (DuckDB → BigQuery)
# ---------------------------------------------------------------------------

def _bq_full_ref(ds_id: str, ds_map: dict) -> str:
    """Return the BigQuery fully-qualified table reference for a CuteBI dataset ID."""
    return ds_map.get(ds_id, f"{BQ_PROJECT}.{BQ_DATASET}.{ds_id.replace(' ', '_')}")

def transform_sql_for_bq(sql: str, ds_map: dict) -> str:
    """
    Convert DuckDB-flavored SQL generated by the CuteBI frontend into
    BigQuery Standard SQL.  ds_map maps ds_id → BQ fully-qualified table path.
    ...
    Transformations applied (in order):
      ...
      5. TRY_CAST          →  SAFE_CAST
      6. date_part(...)    →  EXTRACT(... FROM ...)
    ...
    sql = _re.sub(r'((?:LEFT|RIGHT|INNER|FULL OUTER|FULL|CROSS)?)\s*JOIN\s+"([^"]+)"', _sub_join, sql)

    # 3. EXCLUDE → EXCEPT  (DuckDB star-exclude not supported in BQ)
    sql = sql.replace(' EXCLUDE (', ' EXCEPT (')

    # 4. All remaining double-quoted identifiers → backticks
    sql = _re.sub(r'"([^"]*)"', r'`\1`', sql)

    # 5. TRY_CAST → SAFE_CAST
    sql = sql.replace('TRY_CAST(', 'SAFE_CAST(')

    # 6. date_part('period', x)  →  EXTRACT(PERIOD FROM x)
    for period in ('month', 'year', 'quarter', 'week', 'day'):
        sql = _re.sub(
            rf"date_part\('{period}',\s*",
            f"EXTRACT({period.upper()} FROM ",
    ...
    # 8. Coerce numeric string literals in equality and IN comparisons to avoid BQ's strict
    ...
    # Also coerce the left-hand side of IN (...) operators generated by Slicers
    sql = _re.sub(
        r'(`(?:[^`]+`\.`)?[^`]+`)\s+IN\s+\(',
        r"CAST(\1 AS STRING) IN (",
        sql, flags=_re.IGNORECASE
    )
...
async def run_query(query_request: dict, db: Session = Depends(database.get_db)):
    ...
    if bq_client:
        ...
        if ds_map:  # At least one BQ dataset is registered → use BQ
            bq_sql = transform_sql_for_bq(sql, ds_map)
